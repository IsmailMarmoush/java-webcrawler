...
public class PathFindingExamplesTest {
	...
	public WeightedPath findCheapestPathWithDijkstra(final Node nodeA,
			final Node nodeB) {
		// START SNIPPET: dijkstraUsage
		PathFinder<WeightedPath> finder = GraphAlgoFactory
				.dijkstra(Traversal.expanderForTypes(ExampleTypes.MY_TYPE,
						Direction.BOTH), "cost");
	...
	public void shortestPathExample() {
		...
		PathFinder<Path> finder = GraphAlgoFactory
				.shortestPath(Traversal.expanderForTypes(ExampleTypes.MY_TYPE,
	...
	private Relationship createRelationship(final Node start, final Node end,
		...
		return setProperties(
				start.createRelationshipTo(end, ExampleTypes.MY_TYPE),
				properties);
	}
	...
	private <T extends PropertyContainer> T setProperties(final T entity,
			final Object[] properties) {
